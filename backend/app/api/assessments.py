...
@router.post("/start")
def start_assessment(assessment_id: str, cache: redis.Redis = Depends(get_redis_client)):
...
def submit_answer(assessment_id: str, payload: AnswerPayload, cache: redis.Redis = Depends(get_redis_client)):
    key = f"assessments:active:{assessment_id}"
    try:
...
def finalize_assessment(assessment_id: str, cache: redis.Redis = Depends(get_redis_client)):
    key = f"assessments:active:{assessment_id}"
...
def get_assessment_result(assessment_id: str):
    ...
    return {"message": "Assessment state retrieved", "assessment_id": assessment_id, "state": result}
