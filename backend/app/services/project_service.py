...
def evaluate_project(project_id: str, evaluation: dict) -> dict:
    ...
    eval_reqs = {r["requirement_id"]: r["status"] for r in evaluation.get("requirements", [])}
    ...
    if backend_passed:
        ...
        for skill_eval in evaluation.get("skills_demonstrated", []):
            ...
            if not (0.0 <= score <= 1.0):
                raise ValueError(f"Skill score {score} for {skill_id} is out of bounds [0.0, 1.0]")
            if not (0.0 <= confidence <= 1.0):
                raise ValueError(f"Skill confidence {confidence} for {skill_id} is out of bounds [0.0, 1.0]")
            ...
            state_repo.update_learner_proficiency(learner_id, skill_id, new_prof, confidence)
