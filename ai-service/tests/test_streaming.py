...
async def test_tutor_tools_direct_invocation():
    ...
    with patch.object(BackendClient, '_safe_post', new_callable=AsyncMock) as mock_post:
        ...
        res2 = await record_mistake.ainvoke({"learner_id": "L1", "node_id": "n1", "concept": "X", "description": "Y"})
        assert res2["status"] == "recorded"

async def mock_astream_events_normal(*args, **kwargs):
    chunk1 = MagicMock()
    chunk1.content = "Hello "
    yield {"event": "on_chat_model_stream", "name": "agent_node", "data": {"chunk": chunk1}}
    
    chunk2 = MagicMock()
    chunk2.content = "world."
    yield {"event": "on_chat_model_stream", "name": "agent_node", "data": {"chunk": chunk2}}
...
@patch("app.api.chat.orchestrator_graph.astream_events", side_effect=mock_astream_events_normal)
...
@patch("app.api.chat.orchestrator_graph.astream_events", side_effect=mock_astream_events_tool)
